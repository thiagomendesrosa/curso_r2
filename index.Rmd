...
link-citations: yes
fontsize: 18pt
description: Minicurso de R aplicado a manipulação de microdados da PDAD 2018
---
...
- Estudantes que procuram utilizar o R para realizar análises com microdados.
...
- Versatilidade para integração com outras linguagens e ferramentas. Já é possível rodar python diretamente no RStudio, misturando códigos nas duas linguagens;
...
- O R é preterido por programadores no desenvolvimento de aplicações (por exemplo, sua linguagem não se aproxima tanto da linguagem utilizada por esses profissionais quanto o python);
...
- A instalação de alguns pacotes e ferramentas pode ser complicado, exigindo algum conhecimento mais avançado dos usuários conforme a plataforma utilizada;

- Devido aos itens anteriores, a reprodutibilidade de alguns scripts pode ficar comprometida. Guardar informações da sessão em que o código foi executado pode mitigar essa situação (versão do R e dos pacotes utilizado).


## RStudio

O RStudio é um ambiente integrado de desenvolvimento (*integrated development environment* -- IDE) para a linguagem R (dentre outras). Ele ajuda a organizar os trabalhos desenvolvidos, dividindo os conteúdos em janelas. Ele, por exemplo, tem recursos que facilitam a escrita dos códigos (com atalhos de teclado e o recurso de auto completar). Além disso, possibilita que algumas funções sejam utilizadas via *point-and-click*. Acesse  [essa página](https://csgillespie.github.io/efficientR/set-up.html#rstudio) para maiores informações (em inglês).

Para baixar o RStudio, acesse [este site](https://www.rstudio.com/products/rstudio/download/preview/).

Um recurso muito importante do RStudio é a possibilidade de criar projetos. Projetos conduzidos com controle de versão, por meio do [Github](https://github.com/), são os mais comuns.

## GitHub

O **GitHub** é uma plataforma online de compartilhamento e armazenamento de códigos. Os projetos do GitHub são baseados no `git`, uma ferramenta de versionamento de software. Para realizar a instalação, acesse o site do [Git](https://git-scm.com/downloads).

O git permite gerenciar e versionar projetos, de modo a mantê-los organizados e atualizados, sem o risco de perder informações acidentalmente ou por alguma falha técnica. Outra vantagem é a característica colaborativa da plataforma, permitindo o acesso aos códigos de qualquer lugar, via internet, por outros pesquisadores. Se o repositório for público, qualquer pessoa pode contribuir com os projetos, proporcionando ganhos a todos os futuros usuários. Se for privado, você pode determinar quem poderá acessar e colaborar com o projeto.

Caso você não tenha conta, aproveite essa oportunidade para criar uma no [Github](https://github.com/). Vale lembrar que todos os pacotes desenvolvidos no R ficam armazenados no GitHub e são de acesso público para qualquer usuário. Assim, você pode consultar a programação de todas as funções existentes no R *(nada de caixa preta!)*.
...
Por fim, baixe e instale o [RStudio](https://www.rstudio.com/products/rstudio/download/preview/).


## Estrutura do RStudio

Após instalar o R, o Rtools e o RStudio, abra o RStudio.

O RStudio estará dividido em quatro painéis (ou três, caso você não tenha nenhum script aberto): o superior à esquerda apresenta os scripts abertos para uso; o inferior à esquerda apresenta o console, local em que os comandos e seus resultados são apresentados. Nesse painel há também uma aba para o terminal do Linux; o superior direito apresenta o ambiente, com os objetos ativos na sessão. Há mais quatro abas, com o histórico dos comandos executados, as conexões, *build* (para desenvolvimentos) e o Git (se você estiver no ambiente de projetos); por fim, o inferior direito apresenta o diretório de trabalho, com as pastas e arquivos. Há ainda mais quatro abas, que apresentam os *plots* gerados, os pacotes disponíveis na biblioteca, a janela de ajuda e a janela de visualização (utilizadas por alguns pacotes para mostrar conteúdos da web).
...
Alguns pacotes, quando não disponíveis no CRAN (por ainda estar em desenvolvimento ou não terem sido submetidos), podem ser instalados diretamente do `Github`, com a função `devtools::install_github()`.

Note que o caractere `#` é utilizado para escrever comentários na codificação. Documentar bem seu código é fundamental para as pessoas que trabalham com você entendam o que foi realizado. Além disso, é fundamental para que uma tarefa que dependa desse código possa ser executada ou alterada por outro pesquisador. Para comentar uma série de linhas ao mesmo tempo, utilize o atalho `Ctrl`+`Shift`+`C` após selecioná-las.
...
As funções do tipo `.C()`, `.Call()`, `.Fortran()`, `.External()`, ou `.Internal()` and `.Primitive()` chamam códigos compilados, sendo necessário olhar o código fonte.

Além disso, no RStudio, você conta com as *cheatsheets*, na janela superior de ajuda (`Help`). Você poderá baixar diversos pdfs com "colas" para as principais operações realizadas na análise de dados.
...
Como alternativa, você pode acessar o Github do pacote e verificar a função diretamente do arquivo R. Por exemplo, as funções do pacote `base` estão disponíveis [aqui](https://github.com/SurajGupta/r-source/tree/master/src/library/base/R). Para uma discussão aprofundada sobre o tema, acesse [esse site](https://adv-r.hadley.nz/).
...
Vamos iniciar a atividade de manipulação de base de dados carregando a Pesquisa Distrital por Amostra de Domicílios 2018. A PDAD é uma pesquisa amostral conduzida pela Codeplan a cada dois anos, com representatividade para 31 Regiões Administrativas^[Em 2019, foram criadas duas novas Regiões Administrativas no Distrito Federal. A Lei Complementar nº 958, de 20 de dezembro de 2019 estabeleceu as poligonais de todas as regiões do DF.], tendo por objetivo traçar as principais características dos domicílios e pessoas destas localidades.

Vamos carregar a pesquisa de três maneiras diferentes. A PDAD 2018 está disponível, no formato `.csv` no site da Codeplan, neste [endereço](http://www.codeplan.df.gov.br/microdados-pdad-2018/). São cinco arquivos: dois deles contendo os microdados, referentes aos domicílios e aos moradores; dois arquivos de dicionário, um em formato `.xlsx` e outro em `.pdf`; e o questionário. Em 2020, a Codeplan fez um ajuste nos dados para divulgar informações para as 33 Regiões Administrativas existentes naquele ano do DF, estando disponíveis os mesmos arquivos.
...
Vamos criar um projeto para fazermos nossas análises. Acesse sua conta no [github](https://github.com/) e crie um novo repositório. Depois disso, vamos criar um novo projeto no RStudio. Para isso, clique em `File` -> `New project` -> `Version Control` ->`Git`. No campo `Repository URL`, cole a url do projeto que você acabou de criar. 

<img src="figuras/git.png" style="width: 800px;display: block;margin-left: auto;margin-right: auto "/>

Crie uma pasta chamada `dados`, dentro do diretório do projeto, e salve os arquivos da pesquisa dentro dela.

Feito isso, vamos agora carregar os dados no R de diferentes métodos.

## Método point-and-click

Para isso, clique em `Import Dataset`, no atalho do painel `Environment`. O RStudio vem configurado com duas opções para você importar os dados. Vamos ver a primeira delas: `From Text(base)`.

<img src="figuras/import1.png" style="width: 800px;display: block;margin-left: auto;margin-right: auto "/>

Nesta primeira opção, você seleciona o arquivo `.csv` salvo no passo anterior e o R já apresenta a estrutura dos dados a ser carregada. Marque a opção "yes" em `Heading` e desmarque a opção `String as factors` (se ela estiver marcada). Caso deseje, altere o nome do arquivo e clique em importar.
...
```{r}
dom2018_31ras <- read.csv2("F:/OneDrive/Doutorado/Monitoria FGV/2020/curso_r2/dados/dom2018_31ras.csv")
```
...
```{r}
pdad_2018_moradores <- readr::read_delim("http://www.codeplan.df.gov.br/wp-content/uploads/2020/06/mor2018_31ras.csv", 
...
Note que você pode excluir todos os objetos do ambiente, listando-os com a função `ls()`. Vamos também definir o diretório padrão em que estamos trabalhando. O RStudio server, por padrão, utilizará a pasta de instalação. Quando você inicia um projeto, o diretório padrão é o do projeto. Para consultar o diretório ativo no momento, utilize a função `getwd()`.

```{r}
getwd() # Consultar diretório de trabalho ativo
```

Vamos alterar para a pasta `dados`, criada juntamente com o nosso projeto.
...
pdad_2018_moradores <- data.table::fread("http://www.codeplan.df.gov.br/wp-content/uploads/2020/06/mor2018_31ras.csv",dec = ",",encoding = "Latin-1")
...
## Manuseando as bases de dados

Existem algumas funções úteis para verificar a estrutura dos dados da nossa tabela carregada.

```{r}
#View(tabelas) # Visualizar o objeto tabelas
class(pdad_2018_moradores) # Verificar a classe do objeto
names(pdad_2018_moradores) # Verificar o nome das colunas carregadas
head(pdad_2018_moradores) # Verificar as primeiras linhas da tabela
tail(pdad_2018_moradores) # Verificar as últimas linhas da tabela
str(pdad_2018_moradores) # Verificar as classes das colunas
dplyr::glimpse(pdad_2018_moradores) # Outra opção para checar as classes
nrow(pdad_2018_moradores) # Consultar o número de linhas
ncol(pdad_2018_moradores) # Consultar o número de colunas
```

O objeto do tipo `data.frame`, que é o formato do objeto *tabelas*, será o foco da nossa aula. Esse objeto é como se fosse uma matriz, em que as linhas representam as observações e as colunas as variáveis. Como em uma matriz, você pode acessar as linhas e as colunas com o operador `[,]` logo após o nome do objeto. À esquerda, indica-se a posição da linha, enquanto, à direita, indica-se a coluna. Você pode utilizar a posição numérica ou o nome da coluna. Repare que a função `names()` indica, além dos nomes, a posição das colunas. Elas ficam entre colchetes, apresentadas em ordem crescente. Outra maneira para acessar uma variável específica é utilizando o caractere `$` logo após o nome do `data.frame`. Confira algumas formas de acessar essas informações abaixo. 


```{r}
pdad_2018_moradores$A01ra # Consultar apenas a coluna "A01ra"
pdad_2018_moradores[,2] # Outra maneira de consultar a coluna "A01ra"
pdad_2018_moradores[,"A01ra"] # Mais uma maneira de consultar a coluna "A01ra"
pdad_2018_moradores[1:2,1:2] # Selecionar apenas as duas primeiras linhas e colunas
pdad_2018_moradores[1:2,]$A01ra # Selecionar apenas as duas primeiras linhas da coluna "A01ra"
pdad_2018_moradores[c(1,7,10:12),c(1,3)] # Selecionar linhas e colunas distintas
```

Para fazer uma tabulação simples, utilize a função `table()`.

```{r}
table(pdad_2018_moradores$A01ra) # Tabular a coluna
table(pdad_2018_moradores[,2]) # Tabular a coluna
table(pdad_2018_moradores[,"A01ra"]) # Tabular a coluna
```

Agora vamos aprender a filtrar os dados. Isso pode ser feito da seguinte maneira.

```{r}
pdad_2018_moradores[pdad_2018_moradores$A01ra==1,4]
pdad_2018_moradores[pdad_2018_moradores[,2]==1,]$A01ra
```
...
pdad_2018_moradores %>%
  # Filtrar apenas linhas do Plano Piloto
  dplyr::filter(A01ra==1) %>%
  # Selecionar apenas a coluna da RA
  dplyr::select(A01ra)
```
...
```{r}
# Alterar o nome da coluna "A01ra" para RA
colnames(pdad_2018_moradores)[2] <- "RA"

# Consultar os nomes
names(pdad_2018_moradores)

# Retornar o nome para "A01ra"
names(pdad_2018_moradores)[2] <- "A01ra"

# Alterar o nome com o dplyr
pdad_2018_moradores %>%
  # Alterar os nomes das colunas
  dplyr::rename(RA=A01ra) %>% 
  # Selecionar a coluna renomeada
  dplyr::select(RA) %>% 
  # Manter somente os valores distintos
  dplyr::distinct() %>% 
...
Você também poderia carregar o dicionário pelo painel `Environment`, `Import Dataset` com a opção `From Excel`. Note que o R também tem funções auxiliares para carregar informações do `SPSS`, `SAS` e `Stata`.
...
Vamos agora ver como criar uma nova variável, no formato fator, atribuindo *label* (rótulo) aos valores. Faremos isso com as Regiões Administrativas de residência dos respondentes da pesquisa. Isso pode ser feito, dentro do pacote `dplyr`, com as funções `mutate` ou `transmute`. A primeira cria uma nova variável no banco de dados, mantendo todas as demais, enquanto a segunda mantém somente as variáveis que estão sendo criadas dentro da função.
...
Agora que já aprendemos a carregar e manipular as bases de dados, tente carregar a base de domicílios.

```{r, include=FALSE}
# Carregar a base completa de domicílios
pdad_2018_domicilios <- data.table::fread("http://www.codeplan.df.gov.br/wp-content/uploads/2020/06/dom2018_31ras.csv",dec=",",encoding="Latin-1",data.table=F)
```

```{r}
# Carregar as informações do dicionário
dic_domiclios <- readxl::read_excel("dados/Dicionario_de_Variaveis_PDAD_2018.xlsx",
                                    skip = 1, sheet = 1)
# Criar um objeto com os rótulos
...
- `[Cc]urso` para reconhecer "curso" com o primeiro "c" em maiúsculo ou minúsculo.
...
Vez ou outra nos deparamos com bases de dados que contêm variáveis do tipo data. Fora da família `tidyverse`, temos o pacote `lubridate`, que facilita muito a manipulação deste tipo de dado. Na PDAD 2018, temos a variável `datavisita`, que traz o dia em que as informações foram coletadas no domicílio. Vamos utilizar essa variável para manipular datas.
...
Com o R, é possível fazermos mapas com relativa facilidade. Para tanto, precisaremos de informações geográficas dos mapas que queremos desenhar. Como estamos utilizando a PDAD 2018, iremos fazer um mapa utilizando as poligonais da pesquisa. Vamos aprender a usar o R para baixar informações da internet diretamente no computador.
...
Após baixar os dados, vamos manipular os dados para criar inflatores para a atualização dos valores monetários.
...
Agora, temos um inflator para atualizar os valores monetários da PDAD 2018 para o mês mais recente da tabela (dezembro de 2019). Iremos utilizar essa informação mais tarde, para construirmos a renda em valores atualizados.
...
inflacao_long %>%
  ...
  # Filtrar para o último ano (de dezembro a dezembro)
...
Neste curso iremos aprender a declarar o plano amostral da PDAD 2018 no R, utilizando o pacote `survey`. Vamos utilizar também o pacote `srvyr`, que roda o pacote `survey` sob a lógica do pacote `dplyr`, para calcular as estimativas.
...
Pronto! Já temos o objeto com as informações da PDAD que iremos trabalhar no restante deste curso. Vamos testar estimando o total de pessoas com 18 anos ou mais de idade no Distrito Federal. O nosso objeto base agora será o `amostra`, com o qual utilizaremos o pacote `srvyr`. Com a parâmetro `vartype='ci'`, obtemos as estimativas dos intervalos de confiança. 
...
salario2 <- vars_relatorio %>%
  ...
  srvyr::summarise(n=survey_mean(na.rm=T,vartype = "ci")) %>%   # Retirar NA
  na.omit
...
Após ser escrito o relatório, precisamos compilar o documento, que é realizado com o auxílio dos pacotes `knitr` e `pandoc`. O primeiro deles cria um arquivo com os elementos textuais e do R, no formato `.md`, que posteriormente é convertido para o formato desejado com o segundo pacote.
...
Rode o arquivo de exemplo, clicando no botão `knit`, para verificar o resultado. 
O RStudio pedirá para você indicar o nome e o local do arquivo a ser criado. Toda vez que você "tricotar" o documento, o R salva e executa todos os passos elencados anteriormente automaticamente. 
...
Para configurar suas credenciais no github, digite no terminal os seguintes comandos:

- git config --global user.name "seu nome de usuário"
- git config --global user.email "seu nome de e-mail"

Se quiser configurar sua senha, utilize o comando abaixo:
...
Com ela, verificamos todos os objetos que foram modificados, removidos e/ou adicionados no diretório do projeto. A partir dele, devemos informar o que desejamos fazer com cada um deles. Podemos ignorá-los com o botão `Ignore`, para, neste caso, indicarmos que não desejamos que essa informação seja armazenada no repositório. 
...
Este foi o minicurso de R aplicado a manipulação de microdados da PDAD 2018. O R é uma ferramenta muito ampla, que permite diversas outras aplicações, principalmente na área estatística. Aqui, foi apresentada uma modesta parcela das funcionalidades que esta ferramenta pode oferecer. Ainda há muito mais a ser explorado. Utilizar o R constantemente fará com que você acumule habilidades e avance cada vez mais na utilização desta ferramenta.
